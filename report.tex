...
\section{Introduction}
	...
	In their case, as they are designing buildings, having a way to visualize the 3D models generated by their programs is the minimum required feature.
	...
	%TODO desktop to web bullshit
	%%%% 
	IDEs for GD still depend on a specific OS and on the installation of a software package.
	If the architect wants to make a quick test for a design, he is only able to do so in a computer that has one IDE installed.
	The number of computers that have an IDE installed is not equal to the number of computers the architect has access. 
		Well, he has his laptop where he has the IDE installed. Better access to the IDE lets people try it without compromise.
	
	While these IDEs allow architects to do GD, they still depend on installing software on their computers that may only work on a specific OS.
	If the architect wants to test a GD design, he may be slowed by having to go to the computer that has the IDE installed.
	If he does not have one computer with the IDE installed the problem is worse as he needs to download and install it.
	
	While these IDEs allow architects to do GD, they still depend on installing software on their computers that may only work on a specific OS.
	
	Using an IDE that is dependent on a specific OS or that requires installation means that it can only be used in computers where it has been installed.
	This is a problem when many computers are used with different OSs.
	%%%%

	%TODO explain web page because the reader is so stupid he wouldn't understand anything
	%%%%
	...
	In order to solve these problems, we propose a web-based \ac{ide} for \ac{gd}. 
	Such environment will allow anyone interested in \ac{gd} to edit \ac{gd} programs and visualize their results while not requiring any installation. 
...
\subsubsection{Impromptu.}
	%TODO Why are there graphics in a IDE for live performance?!
...
\subsubsection{Processing.}
	...
	%TODO fucking shrink processing description
