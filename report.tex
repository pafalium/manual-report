...
\section{Conclusions}
	...
	They do not want to make programs like those of software development nor do they have the knowledge required to be able to do it.
	
	\ac{ide}s like Impromptu, IPython, Processing, DesignScript and Rosetta are examples of \ac{ide}s for programming for purposes other than software development.
	...
	With the need for a different \ac{ide} for people doing \ac{gd} and the possibility of running applications in the browser, we proposed that having such an \ac{ide} in the web browser can give more flexibility to people making \ac{gd}.
	
	To have a working \ac{ide} for \ac{gd} in the browser there need to be at least two components, a source code editor and a results visualizer.
	...
	A particular element in making it is the overall responsiveness of the user interface.
	
	
	
	When the IDE is working (after the code editor and results visualizer are done) there is still room for more.
	Things like a backend for running programs in CADs and storing programs in the cloud may be implemented...
